...
目前几乎所有博客内容里的图片都**不是以二进制存储在数据库**的，而是**把图片上传到服务器**然后正文里使用[![img](https://www.cnblogs.com/Gao-yubo/p/undefined)](https://www.cnblogs.com/Gao-yubo/p/undefined)标签引用，这样的博客就可以使用TEXT类型。而BLOB就可以把图片换算成二进制保存到数据库中。
